...
PARAMETER_STRING
  ...
  INSTANCE_ID=$(aws ec2 run-instances --image-id "$VANILLA_IMAGE_ID" --instance-type t4g.micro --iam-instance-profile "Name=$INSTANCE_PROFILE_NAME" --security-group-ids "$SECURITY_GROUP_ID" \
    ...
    --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=AMI_Chef_Setup_${ENVIRONMENT}_${APP_NAME}},{Key=Environment,Value=$ENVIRONMENT},{Key=Service,Value=CKAN}]" \
